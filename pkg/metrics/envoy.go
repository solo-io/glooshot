...
func (c *QueryPoller) GetEnvoySuccessRate(name string, namespace string, metric string, interval string) (float64, error) {
...
func (c *QueryPoller) GetEnvoyRequestDuration(name string, namespace string, metric string, interval string) (time.Duration, error) {
	if c.metricsServer == "fake" {
		return 1, nil
	}
