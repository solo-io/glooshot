...
var _ = Describe("ExperimentClient", func() {
	var (
		namespace string
	)
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: ExperimentCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewExperimentClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs Experiments "+test.Description(), func() {
				ExperimentClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func ExperimentClientTest(namespace string, client ExperimentClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewExperiment(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&Experiment{}))
	...
	Eventually(func() ExperimentList {
		list, err = client.List(namespace, clients.ListOpts{})
		Expect(err).NotTo(HaveOccurred())
		return list
