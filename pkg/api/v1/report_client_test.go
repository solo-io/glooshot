...
var _ = Describe("ReportClient", func() {
	...
	for _, test := range []typed.ResourceClientTester{
		&typed.KubeRcTester{Crd: ReportCrd},
		...
		Context("resource client backed by "+test.Description(), func() {
			...
			BeforeEach(func() {
				...
				client, err = NewReportClient(factory)
				Expect(err).NotTo(HaveOccurred())
			})
			AfterEach(func() {
				test.Teardown(namespace)
			})
			It("CRUDs Reports "+test.Description(), func() {
				ReportClientTest(namespace, client, name1, name2, name3)
			})
		})
	}
})

func ReportClientTest(namespace string, client ReportClient, name1, name2, name3 string) {
	err := client.Register()
	Expect(err).NotTo(HaveOccurred())

	name := name1
	input := NewReport(namespace, name)
	...
	Expect(r1).To(BeAssignableToTypeOf(&Report{}))
